...
fvfm_byday <- ggplot(fvfm_alldays, aes(x = leachate, y = FvFm, color = temp, fill = leachate)) +
  geom_boxplot() +
...
> Duration of experiment had a significant effect (p = 1.957e-07 \*\*\*)
...
# Create the boxplot with custom colors for temp and viridis for leachate
alpha_byday <- ggplot(reg_alldays, aes(x = leachate, y = alpha, color = temp, fill = leachate)) +
  geom_boxplot() +
  facet_grid(. ~ days_of_exposure, switch = "x") +
  theme_bw() +
  labs(
...
# save the plot as a .png
ggsave("../output/alpha_byday_plot.png", plot = alpha_byday, width = 10, height = 6, dpi = 600)
```
...
\% \> Days of exposure had a significant effect (p = 0.04576 \*) on
...
# save the plot as a .png
ggsave("../output/rETR_line_plot.png", plot = alpha_line, width = 10, height = 6, dpi = 600)
```
...
> affected by temperature (p = 0.006489 **), and duration of experiment
> (p = 0.004245** ), but interestingly, not the interaction of the
> two...
